...
#            data = data * signal.hann(nSampWin)[:,NA,NA]
            ...
            # But wave is rebresented by exp(i(k*x-omega*t)). In this case, 
            # if k is positive, negative frequency means westward wave and positive 
            # frequency means eastward wave.
            #
        ...
        freqtimes = 10               #the number of passes of filter in freqency
...
def wk99power(datain, spd, nDayWin, nDaySkip, tim_taper=0.1):
    """space-time spectram WK99
      ...
      Return:
        ...
        power spectrum array (wavenumber, frequency) sorted in ascending order of wavenumber
        and frequency. wavenumber and frequency array can be got by genwavenumber and genfrequency
        method.
    
        """
    #TODO:sort variaous dimension
    #xdim, ydm, tdim = dim
...
def powershape(data, spd, nDayWin):
    ...
    powershape = (nSampWin/2, nlat, nlon-(nlon+1)%2)
...
def calcbackground(power, frequency, lw=None):
    
    nf, nlat, nk = power.shape
    f0 = nf/2
    k0 = nk/2
    ...
    # background spectrum
    ###########################################################
    background = numpy.add.reduce(power, axis=1)

    #argument
    lw = 27   #smooth wave bound (desplay bound)
    freqtimes = 10  #the number of passes of filter in freqency
    stepfreq = [0.1,0.2,0.3,1.]
    wavetimes = [5, 10, 20, 40]
    ...
    i = 1
    while i <= freqtimes:
        background[k0-lw:k0+lw+1,f0+1:] = _smooth121_2d(background[k0-lw:k0+lw+1,f0+1:],axis=1)
        i += 1

    m = 0
    for n in range(f0+1, nf):
        if frequency[n] > stepfreq[m]:
            m = m + 1                
            i = 1
            while i<=wavetimes[m]:
                background[n,k0-lw:k0+lw+1] = _smooth121(background[n,k0-lw:k0+lw+1])
                i += 1
